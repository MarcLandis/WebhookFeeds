...
class FeedItemBase(SQLModel):
    ...
    pubDate: Union[datetime, None] = Field(
        default=None, index=True, title="feed item publication date"
    )
...
class FeedItem(FeedItemBase, table=True):
    id: uuid.UUID = Field(
        default_factory=uuid.uuid4, primary_key=True, title="unique feed item id"
    )
...
class FeedItemPublic(FeedItemBase):
    ...
    feed_id: Union[uuid.UUID, None] = Field(
        default=None, title="feed id to add the item to"
    )
