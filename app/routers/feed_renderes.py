...
def generate_feed(
    ...
    if not feed_db:
        raise HTTPException(status_code=404, detail="feed not found")
    feeditems_db = session.exec(
        select(FeedItem).where(FeedItem.feed_id == feed_id)
    ).all()
    feed: feedendum.Feed = feedendum.Feed(
        title=feed_db.title,
        url=feed_db.link,
    ...
    for feeditem_db in feeditems_db:
        feeditem = feedendum.FeedItem(
            title=feeditem_db.title,
            url=feeditem_db.link,
            content=feeditem_db.description,
            update=feeditem_db.pubDate,
            id=feeditem_db.guid,
        )
