...
async def get_feeditems(
    ...
    feeditems = session.exec(
        select(FeedItem).where(FeedItem.feed_id == feed_id).offset(offset).limit(limit)
    ).all()
    return feeditems
...
async def add_feeditem(
    ...
    feeditem_db = session.exec(
        ...
        .where(FeedItem.guid == feeditem.guid)
    ).first()
    if insert_or_update:
        if feeditem_db:
